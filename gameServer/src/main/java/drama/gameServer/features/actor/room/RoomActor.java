...
public class RoomActor extends DmActor {
    ...
    private void onInnerMsg(RoomInnerMsg msg) {
        ...
        } else if (msg instanceof In_RoomShootResult) {
            onShootResult((In_RoomShootResult) msg);
        } else if (msg instanceof In_RoomShootEnding) {
            onShootEnding((In_RoomShootEnding) msg);
        }
    }

    private void onShootEnding(In_RoomShootEnding msg) {
        for (Entry<String, RoomPlayerCtrl> entry : roomCtrl.getTarget().getIdToRoomPlayerCtrl().entrySet()) {
            entry.getValue().send(msg.getResponse());
        }
    }

    private void onShootResult(In_RoomShootResult msg) {
        for (Entry<String, RoomPlayerCtrl> entry : roomCtrl.getTarget().getIdToRoomPlayerCtrl().entrySet()) {
            entry.getValue().send(msg.getResponse());
        }
    }
    ...
    private void onGmKillRoomMsg(In_GmKillRoomMsg msg) {
        LOGGER.debug("GMå…³é—­æˆ¿é—´,æ‰€æœ‰ç©å®¶æ¸…å‡ºæˆ¿é—´");
        for (String playerId : roomCtrl.getTarget().getIdToRoomPlayer().keySet()) {
    ...
    private void onCheckPlayerAllVoteSearchRoomMsg() {
        ...
        if (roomCtrl.checkPlayerFinishVoteSearch(voteNum)) {
            // é€šçŸ¥æ‰€æœ‰ç©å®¶æŠ•ç¥¨ç»“æœå’Œæœ€ç»ˆæŠ•ç¥¨é€‰ä¸­çš„çº¿ç´¢
    ...
    private void onCheckPlayerAllReadyMsg(In_CheckPlayerAllReadyRoomMsg msg) {
        if (!roomCtrl.checkAllPlayerReady()) {
            ...
            // æ²¡æœ‰è¾¾åˆ°é™åˆ¶çš„å‡†å¤‡æ—¶é—´,è¿˜ä¸èƒ½ä¸¾æ‰‹
            LOGGER.debug("æ²¡æœ‰è¾¾åˆ°é™åˆ¶çš„å‡†å¤‡æ—¶é—´,è¿˜ä¸èƒ½ä¸¾æ‰‹");
            return;
        } else if (!roomCtrl.checkPlayerFinishSearch()) {
            // æœ‰ç©å®¶æ²¡æœ‰æœå®Œè¯
            LOGGER.debug("æœ‰ç©å®¶æ²¡æœ‰æœå®Œè¯");
            return;
        }
        //æ‰€æœ‰äººéƒ½å·²ç»ä¸¾æ‰‹å‡†å¤‡å¥½äº†,å¹¶ä¸”é…ç½®é‡Œè¿˜æœ‰ä¸‹ä¸€ä¸ªç¯èŠ‚
        roomCtrl.setNextStateAndTimes();
        //é€šçŸ¥æ‰€æœ‰ç©å®¶æˆ¿é—´çŠ¶æ€å‘ç”Ÿæ”¹å˜
        for (String playerId : roomCtrl.getTarget().getIdToRoomPlayer().keySet()) {
            In_PlayerOnSwitchStateRoomMsg in_playerOnSwitchStateRoomMsg = new In_PlayerOnSwitchStateRoomMsg(playerId, roomCtrl.getTarget());
            sendToWorld(in_playerOnSwitchStateRoomMsg);
        }
    ...
    private void onPlayerDisconnectedRoom(In_PlayerDisconnectedRoomMsg msg) {
        ...
        if (roomCtrl.containsPlayer(playerId)) {
            ...
            LOGGER.debug("ç©å®¶ä¸åœ¨æˆ¿é—´ä¸­,é€€å‡ºæˆ¿é—´å¼‚å¸¸ å¿½ç•¥: playerId={}, roomId={}", playerId, roomId);
        }
    }
            ...
            try {
                switch (cm_room.getAction().getNumber()) {
                    ...
                    case Action.AUCTION_RESULT_VALUE:
                        onAuctionResult();
                        break;
                    case Action.CAN_SHOOT_VALUE:
                        onCanShoot();
                        break;
                    case Action.SHOOT_VALUE:
                        onShoot(cm_room.getIsShoot(), cm_room.getPower(), cm_room.getBeRoleId());
                        break;
                    case Action.SHOOT_LIST_VALUE:
                        onShootList();
                        break;
                    case Action.SHOOT_ENDING_VALUE:
                        onShootEnding();
                        break;
                    case Action.CHOICE_VALUE:
                        onChoice(cm_room.getBeRoleId());
                        break;
...
//        }
    ...
    private void onChoice(int beRoleId) {
        _checkRoomContainsPlayer(simplePlayer);
        if (roomCtrl.getRoomState() != RoomStateEnum.CHOICE) {
            LOGGER.debug("æˆ¿é—´çŠ¶æ€ä¸åŒ¹é… playerId={}, RoomStateEnum={}", simplePlayer.getPlayerId(), roomCtrl.getRoomState().toString());
            return;
        }
        roomCtrl.onChoice(simplePlayer.getPlayerId(), beRoleId);
    }

    private void onCanChoice() {
        _checkRoomContainsPlayer(simplePlayer);
        if (roomCtrl.getRoomState() != RoomStateEnum.CHOICE) {
            LOGGER.debug("æˆ¿é—´çŠ¶æ€ä¸åŒ¹é… playerId={}, RoomStateEnum={}", simplePlayer.getPlayerId(), roomCtrl.getRoomState().toString());
            return;
        }
        roomCtrl.onCanChoice(simplePlayer.getPlayerId());
    }

    private void onShootEnding() {
        _checkRoomContainsPlayer(simplePlayer);
        if (roomCtrl.getRoomState() != RoomStateEnum.SHOOT) {
            LOGGER.debug("æˆ¿é—´çŠ¶æ€ä¸åŒ¹é… playerId={}, RoomStateEnum={}", simplePlayer.getPlayerId(), roomCtrl.getRoomState().toString());
            return;
        }
        roomCtrl.onShootEnding(simplePlayer.getPlayerId());
    }

    private void onShootList() {
        _checkRoomContainsPlayer(simplePlayer);
        if (roomCtrl.getRoomState() != RoomStateEnum.SHOOT) {
            LOGGER.debug("æˆ¿é—´çŠ¶æ€ä¸åŒ¹é… playerId={}, RoomStateEnum={}", simplePlayer.getPlayerId(), roomCtrl.getRoomState().toString());
            return;
        }
        roomCtrl.onShootList(simplePlayer.getPlayerId());
    }

    private void onShoot(boolean isShoot, PowerEnum power, int beShootRoleId) {
        _checkRoomContainsPlayer(simplePlayer);
//        if (!roomCtrl.getTarget().getRoleIdToPlayerId().keySet().contains(beShootRoleId)) {
//            LOGGER.debug("è¢«å°„å‡»ç©å®¶ä¸åœ¨æˆ¿é—´ä¸­, roleId={}, roomId={}", beShootRoleId, roomId);
//            throw new BusinessLogicMismatchConditionException("è¢«å°„å‡»ç©å®¶ä¸åœ¨æˆ¿é—´ä¸­ roleId=" + beShootRoleId + ",roomId=" + roomId, EnumsProtos.ErrorCodeEnum.NOT_IN_ROOM);
//        }
        if (roomCtrl.getRoomState() != RoomStateEnum.SHOOT) {
            LOGGER.debug("æˆ¿é—´çŠ¶æ€ä¸åŒ¹é… playerId={}, RoomStateEnum={}", simplePlayer.getPlayerId(), roomCtrl.getRoomState().toString());
            return;
        }
        roomCtrl.onShoot(simplePlayer.getPlayerId(), isShoot, power, beShootRoleId);
    ...
    private void onCanShoot() {
        ...
        roomCtrl.onCanShoot(simplePlayer.getPlayerId());
...
//        }
    ...
    private void onSelectDraft(int draftId, int draftNum) {
        ...
        RoomPlayerCtrl roomPlayerCtrl = roomCtrl.getRoomPlayerCtrl(simplePlayer.getPlayerId());
        if (roomPlayerCtrl.hasSelectDraft()) {
            LOGGER.debug("å·²ç»é€‰æ‹©è¿‡è½®æŠ½äº† playerId={}", simplePlayer.getPlayerId());
            return;
        }
        if (!roomCtrl.canSelectDraft(draftId, draftNum)) {
            String msg = String.format("æ‰‹æ…¢äº†,è¢«åˆ«äººé€‰èµ°äº† playerId=%s, draftId=%s", simplePlayer.getPlayerId(), draftId);
    ...
    private void onVoteSearch(String typeName) {
        ...
        if (roomPlayerCtrl.getVoteSrchTimes() == MagicNumbers.DEFAULT_ZERO) {
            String msg = String.format("æŠ•ç¥¨æœè¯æ¬¡æ•°å·²ç»ç”¨å®Œäº†,è¿™ä¸ªè¯·æ±‚åº”è¯¥è¢«å®¢æˆ·ç«¯æŒ¡ä½çš„! playerId=%s", simplePlayer.getPlayerId());
            LOGGER.debug(msg);
            return;
        }
        if (roomCtrl.hasVoteSearch(roomPlayerCtrl.getRoleId())) {
            String msg = String.format("ç©å®¶å·²ç»æŠ•è¿‡ç¥¨äº†,è¿™ä¸ªè¯·æ±‚åº”è¯¥è¢«å®¢æˆ·ç«¯æŒ¡ä½çš„! playerId=%s", simplePlayer.getPlayerId());
    ...
    private void onVote(int roleId) {
        ...
        if (roomPlayerCtrl.isVoteMurder()) {
            String msg = String.format("å·²ç»æŠ•è¿‡å‡¶,è¿™ä¸ªè¯·æ±‚åº”è¯¥è¢«å®¢æˆ·ç«¯æŒ¡ä½çš„! playerId=%s", simplePlayer.getPlayerId());
            LOGGER.debug(msg);
            return;
        }
    ...
    private void checkPlayerFinishSearchShowHideClueMsg() {
        if (roomCtrl.checkPlayerFinishSearch()) {
            //æ‰€æœ‰äººéƒ½å·²ç»ç”¨å®Œäº†æœè¯æ¬¡æ•°,å¹¶ä¸”æœ‰éšè—é…ç½®,æ”¾è¿›æˆ¿é—´é‡Œ,ç„¶ååŒæ­¥ç»™ç©å®¶
    ...
    private void onOpenDub(int isDub, Action action, int soloNum) {
        ...
        if (roomCtrl.getRoomState() == EnumsProtos.RoomStateEnum.STAGE || roomCtrl.getRoomState() == EnumsProtos.RoomStateEnum.SOLO) {
            ...
            if (action == Action.IS_DUB) {
                //å¦‚æœæ˜¯å°å‰§åœºé…éŸ³,å¿…é¡»è¦åˆ¤æ–­æ‰€æœ‰äººéƒ½å·²ç»é€‰æ‹©è¿‡æ˜¯å¦é…éŸ³
                ...
                //å¦‚æœæ˜¯soloé…éŸ³ç›´æ¥å¹¿æ’­ç»™æ‰€æœ‰äºº
    ...
    private void onReady(EnumsProtos.RoomStateEnum state, int stateTimes, boolean handsDown) {
        _checkRoomContainsPlayer(simplePlayer);
        //æš‚æ—¶é€‰è§’é˜¶æ®µå¼ºåˆ¶åˆ¤æ–­æ˜¯å¦å·²ç»é€‰å®šè§’è‰²,å¯èƒ½æœ‰ä¸ªåˆ«çš„å‰§æœ¬æ²¡æœ‰å¼ºåˆ¶é€‰å®šè§’è‰²
        RoomPlayerCtrl roomPlayerCtrl = roomCtrl.getRoomPlayerCtrl(simplePlayer.getPlayerId());
        if (roomCtrl.getRoomState() == state && roomCtrl.getRoomStateTimes() == stateTimes) {
            //TODO æ ¹æ®çŠ¶æ€è·å–é…ç½®,åº”è¯¥æœ‰ä¸€äº›é™åˆ¶ä¸¾æ‰‹å‡†å¤‡çš„æ¡ä»¶
            if (roomCtrl.getRoomState() == EnumsProtos.RoomStateEnum.SEARCH && roomPlayerCtrl.getSrchTimes() != 0) {
                //å¦‚æœæ˜¯æœè¯é˜¶æ®µ,å¹¶ä¸”æœè¯æ¬¡æ•°æ²¡æœ‰ç”¨å®Œ,ä¸èƒ½ä¸¾æ‰‹å‡†å¤‡
                LOGGER.debug("æœè¯é˜¶æ®µ,å¹¶ä¸”æœè¯æ¬¡æ•°æ²¡æœ‰ç”¨å®Œ,ä¸èƒ½ä¸¾æ‰‹å‡†å¤‡ playerId={},srchTimes={}", simplePlayer.getPlayerId(), roomPlayerCtrl.getSrchTimes());
                return;
            }
            if (roomCtrl.hasChooseRole(roomPlayerCtrl.getRoleId(), simplePlayer.getPlayerId())) {
                ...
                LOGGER.debug("ç©å®¶è¿˜æ²¡æœ‰é€‰å®šè§’è‰²,ä¸èƒ½ä¸¾æ‰‹å‡†å¤‡ playerId={}", simplePlayer.getPlayerId());
                return;
            }
        } else {
            LOGGER.debug("è¯·æ±‚å‡†å¤‡çš„çŠ¶æ€å’Œæˆ¿é—´çŠ¶æ€ä¸åŒ¹é… playerId={}, RequestStateEnum={},RoomStateEnum={},RequestStateTimes={},RoomStateTimes={}", //
    ...
    private void onSelect(int roleId, Action action) {
        ...
        if (!canSelectRoleIds.contains(roleId)) {
            String msg = String.format("è¿™ä¸ªè§’è‰²å·²ç»è¢«é€‰äº† playerId=%s, roleId=%s", simplePlayer.getPlayerId(), roomPlayerCtrl.getRoleId());
