...
import drama.gameServer.features.actor.login.LoginActor;
...
public class DMRootActor extends DmActor {
    ...
    private void _createChild() {
        try {
            ...
            context().watch(context().actorOf(Props.create(LoginActor.class), ActorSystemPath.DM_GameServer_Login));
        } catch (Exception e) {
            LOGGER.error("", e);
            System.exit(-1);
        }
